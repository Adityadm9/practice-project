...
function FifthPage() {
  ...
  const rightContent = (
    <div className="px-6 mb-10 text-center lg:text-start ">
      <h1 className="font-nunito font-bold text-gray-900 text-3xl sm:text-4xl mb-8 mt-10  lg:text-5xl text-center md:text-start">
        Lorem Ipsum is simply
      </h1>
      <p className="font-lora text-sm sm:text-lg text-gray-900 mb-10  text-justify xl:leading-loose">
        Lorem Ipsum is simply dummy text of the printing and typesetting
        industry. Lorem Ipsum has been the industry <br />
        standard dummy text ever since the 1500s, when an unknown <br />
        printer took a galley of type Lorem Ipsum is simply dummy text of the
  ...
  const leftContent = (
    <div className="mb-10">
      <div className="relative dualbtn bg-orange-200 border-white border-4 rounded-3xl justify-between w-24 h-12 z-20 ml-0 hidden xl:flex text-start xl:ml-5 xl:mt-14 2xl:ml-20">
        <button
          className={`left text-3xl ${
            isLeftActive ? "bg-yellow-400" : "bg-orange-200"
          } w-12 rounded-l-3xl text-gray-900`}
          onClick={() => setIsLeftActive(true)}
        >
          &lt;
        </button>
        <button
          className={`right text-3xl ${
            !isLeftActive ? "bg-yellow-400" : "bg-orange-200"
          } w-12 rounded-r-3xl text-gray-900`}
      ...
      <img
        src={image}
        alt=""
        className="border-8 border-white border-solid rounded-3xl drop-shadow-md z-20 h-auto w-96 mx-auto lg:-mt-16 lg:w-auto"
      />
      <img
        src={circle}
        alt=""
        className="xl:h-28 xl:w-28 xl:-mt-16 xl:ml-7 2xl:ml-20 hidden xl:flex"
      />
    </div>
  );
